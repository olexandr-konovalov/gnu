GNUDATA:=[];

GNUDATA[2016]:=[6538, "precomputed using GrpConst package"];
GNUDATA[2024]:=[46, "precomputed using GrpConst package"];
GNUDATA[2025]:=[63, "precomputed using GrpConst package"];
GNUDATA[2040]:=[175, "precomputed using GrpConst package"];
GNUDATA[2052]:=[205, "precomputed using GrpConst package"];
GNUDATA[2058]:=[91, "precomputed using GrpConst package"];
GNUDATA[2064]:=[222, "precomputed using GrpConst package"];
GNUDATA[2072]:=[40, "precomputed using GrpConst package"];
GNUDATA[2079]:=[13, "precomputed using GrpConst package"];
GNUDATA[2080]:=[1381, "precomputed using GrpConst package"];
GNUDATA[2088]:=[156, "precomputed using GrpConst package"];
GNUDATA[2160]:=[3429, "precomputed using GrpConst package"];
GNUDATA[2304]:=[15756130,"http://dx.doi.org/10.1016/j.jalgebra.2013.09.028"];
GNUDATA[3600]:=[4484, "precomputed using GrpConst package"];
GNUDATA[3888]:=[20065, "precomputed using GrpConst package"]; ];
GNUDATA[10000]:=[4728, "precomputed using GrpConst package"];
GNUDATA[15120]:=[22758, "precomputed using GrpConst package"];
GNUDATA[18480]:=[5397, "precomputed using GrpConst package"];
GNUDATA[50004]:=[189, "precomputed using GrpConst package"];
GNUDATA[50008]:=[112, "precomputed using GrpConst package"];
GNUDATA[50016]:=[1208, "precomputed using GrpConst package"];
GNUDATA[50024]:=[197, "precomputed using GrpConst package"];
GNUDATA[50031]:=[16, "precomputed using GrpConst package"];
GNUDATA[50032]:=[177, "precomputed using GrpConst package"];
GNUDATA[50040]:=[695, "precomputed using GrpConst package"];
GNUDATA[50058]:=[1568, "precomputed using GrpConst package"];
GNUDATA[50064]:=[984, "precomputed using GrpConst package"];
GNUDATA[50072]:=[41, "precomputed using GrpConst package"];
GNUDATA[50078]:=[30, "precomputed using GrpConst package"];
GNUDATA[50080]:=[1434, "precomputed using GrpConst package"];
GNUDATA[50085]:=[13, "precomputed using GrpConst package"];
GNUDATA[50088]:=[39, "precomputed using GrpConst package"];
GNUDATA[50096]:=[177, "precomputed using GrpConst package"];
GNUDATA[50120]:=[126, "precomputed using GrpConst package"];
GNUDATA[50128]:=[244, "precomputed using GrpConst package"];
GNUDATA[50531]:=[5, "precomputed using GrpConst package"];
GNUDATA[50841]:=[13, "precomputed using GrpConst package"];
GNUDATA[50949]:=[60, "precomputed using GrpConst package"];
